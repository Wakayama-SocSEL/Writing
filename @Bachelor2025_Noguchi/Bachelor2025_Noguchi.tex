...
ソフトウェアの性能効率性は，JIS X 25010:2013（ISO/IEC 25010:2011）にシステム/ソフトウェア製品品質を図る尺度の一つとして記述され，システム全体の品質，さらにはユーザ体験や運用コストに直結する重要な要素である．\cite{performance1}\cite{performance2}\cite{negative}．\todo{ISO/IECの性能効率性の引用必要かも}
一般に，性能効率性の向上にはプロファイラといった，動的解析が用いられ，ボトルネックの特定と改善が行われる．しかし、システムレベルの大規模なテストによる性能評価は，実行環境の構築や測定に多大なコストを要する\todo{引用}だけでなく，開発の後期工程で問題が発覚した場合，修正による影響範囲が広く\todo{引用}，実装が進むほどに開発者に対して広範な知識や経験を要するようになる．また，性能効率性を向上するためのプログラム変更では，可読性や保守性といった他の品質を損なうリスク（性能改善と他品質のトレードオフ）も指摘されている\cite{negative}．
...
\todo{（ユニットテストの説明）〜を行う}ユニットテストや，それを利用したマイクロベンチマークを用いることで、コンポーネント単位の微小な範囲でパフォーマンスの変化を早期に評価することが可能となる\todo{引用}．
...
マイクロベンチマーク共有サービスは，マイクロベンチマークを開発者がWeb上で実行・共有するプラットフォームとして機能するオンラインサービスである．例として，JavaScriptを対象としたJsPerf\footnote{JsPerf: \url{https://jsperf.app/}}やMeasureThat.net\footnote{MeasureThat.net: \url{https://measurethat.net/}}が挙げられる．これらのサービスは，ブラウザ上での実行環境を提供するとともに，世界中の開発者が投稿したソースコード断片とその測定結果を公開している．
...
既存の実装から性能が向上する可能性のある低速な箇所を特定することには，大きく分けて二つの課題が存在する．一つはプロファイラやマイクロベンチマークを用いた従来の性能評価手法が，プログラムの動的実行に依存している点である．

性能評価のために実際にコードを動作させるためには，対象がコンパイル可能であり，かつ実行に必要な依存ライブラリやテストデータが整備された「実行可能状態」にあることが前提となる．
しかし，開発の初期段階やリファクタリングの検討段階において，断片的なソースコードに対して即座に実行環境を構築することは容易ではなく，また，実装が進行するにつれて，性能効率性を向上するためのプログラム変更は複雑になりやすい\cite{complicate}ことが示されていることから，動的解析に頼る手法では，実装の最中にリアルタイムで低速な記述を検出し，開発を支援することが困難であるという制約がある．

もう一つは，本研究が着目するマイクロベンチマーク共有サービスにおいて，蓄積されているコード片は，特定の開発者が特定の文脈で局所的な実装方法を比較するために作成したものであるという点である．
これによって，特定の実行環境やデータ入力に対してのみ有意な差が生まれるようなコンテキストへの依存が発生している可能性や，測定のための補助的な変数宣言といった，実行速度を比較している処理の本質と無関係な記述が混在している可能性がある．
マイクロベンチマークという，極めて小さいコード片において，低速である本質的な要因が構造化されていないことから，膨大な公開データから「低速な要因」を抽出するためには，ソースコードの構造的・意味的な特徴をとらえ，本質的なパターンを識別する解析手法が必要となる．
...
Turcotte ら\cite{DrAsync}は，JavaScript 言語における非同期処理に注目した性能アンチパターンを定義し，静的解析エンジンであるCodeQL\cite{ql}を利用したアンチパターンの検出と，動的解析を利用したパフォーマンスの監視を組み合わせ，修正可能な性能アンチパターンの検出を行った．\todo{この研究の貢献・静的解析の利用可能性を示した上で，この研究との差を示す}
