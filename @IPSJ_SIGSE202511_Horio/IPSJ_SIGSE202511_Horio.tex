...
従来研究において，Yangら\cite{10.1145/2724660.2724674}および橋谷ら\cite{hashitani2022scratch}は，リミックスを行うことでユーザが使用するブロックの種類数やCTスコアが増加し，結果としてユーザのCTスキルが向上することを示した．さらに事前分析により，ユーザのCTスキルに寄与するリミックス元作品は，ユーザのCTスキルによって異なることが明らかとなった．しかし，現状のScratchにおいては，作品を探す手段がキーワード検索に限られており，ユーザのCTスキルに適した作品を見つけることは困難である．この課題を解決するための有効な手段の一つとして，ユーザのCTスキルに応じたリミックス元作品の推薦が考えられる．その実現には，推薦に有効な情報や基準を含むモデルが必要となる．そこで本研究では，強化学習を用いたリミックス元作品の推薦モデルを提案する．

本研究は，マルコフ決定過程(MDP)に基づき，ScratchユーザのリミックスによるCTスキルの学習過程をモデル化する．これにより，ユーザのCTスキルの状態に応じた最適なリミックス作品の選択を推奨し，効果的な学習を支援する．

本研究では2つのReseach Question（RQ）に回答する．

\begin{description}
  \item[RQ1：ユーザの熟練度によってリミックスの影響は異なるか] 
  \item[RQ2：強化学習によるリミックス元作品のCTスコア予測は可能か] 
\end{description}
...
%   \item $S$：状態の集合
%   \item $A$：行動の集合
%   \item $P(s′∣s,a)$：状態s∈Sにおいて行動a∈Aを取ったときに，次の状態がs′∈Sになる確率（状態遷移確率）
%   \item $R(s,a)$：状態sで行動aを取った際に得られる報酬
