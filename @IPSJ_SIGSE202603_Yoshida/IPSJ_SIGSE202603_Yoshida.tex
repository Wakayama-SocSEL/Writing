...
\begin{abstract}
近年のソフトウェア開発において，ライブラリの保守はエコシステムの健全性に直結する．しかし，APIの肥大化に伴う保守コストの増大が課題となっている．そこで，不要なメソッドを特定・削除することは不可欠であり，過去の削除履歴を学習して削除候補を推薦するモデルの構築が期待される．しかし，バージョン管理システムの履歴にはリネーム等のリファクタリングがノイズとして含まれる．既存ツールは，変更のコミットが離れている場合に検出が困難になる限界が存在する．そこで本研究では，コードクローン検出ツールNILを用いてバージョン間のメソッドを追跡する手法を提案する．NILはタイプ3クローンの検出に加え，包含関係による統合・分割も識別可能であり，コミットの距離に依存せず削除メソッドを特定できる．本研究は，将来的な削除予測モデル構築に向けた高品質なデータセット生成の基盤となるものである．
\end{abstract}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1.0\linewidth]{@Bachelor2025_Yoshida/Yoshida_fig/method_group.pdf}
